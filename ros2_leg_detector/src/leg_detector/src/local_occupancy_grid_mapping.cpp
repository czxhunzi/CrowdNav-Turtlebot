...
class OccupancyGridMapping :public rclcpp::Node
{
    public:
                                ...
                                sync(scan_sub_, non_leg_clusters_sub_, 100)
        {
            ...
            this->declare_parameter<std::string>("local_map_topic", "local_map");

            // 数值类型参数
            this->declare_parameter<double>("local_map_resolution", 0.05);
            this->declare_parameter<int>("local_map_cells_per_side", 200);
            this->declare_parameter<bool>("invalid_measurements_are_free_space", false);
            this->declare_parameter<bool>("unseen_is_free_space", false);
            this->declare_parameter<bool>("use_scan_header_stamp_for_tfs", false);
            this->declare_parameter<double>("shift_threshold", 0.1);
            this->declare_parameter<double>("reliable_inf_range", 10.0);
            this->declare_parameter<double>("cluster_dist_euclid", 0.15);
            this->declare_parameter<int>("min_points_per_cluster", 6);
            ...
            // buffer_ = std::make_shared<tf2::BufferCore>(this->get_clock());
